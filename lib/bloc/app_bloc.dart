...
import 'package:virtuozy/data/services/log_service.dart';
    ...
    try {
      ...
      if (uid.isEmpty) {
        ...
        if (userType.isUnknown) {
          emit(
              state.copyWith(authStatusCheck: AuthStatusCheck.unauthenticated));
        } else {
          if (userType.isStudent) {
            ...
            if (user.userStatus == UserStatus.auth) {
              emit(state.copyWith(
                  authStatusCheck: AuthStatusCheck.authenticated,
      ...
      LogService.sendLog(TypeLog.errorInitApp,stack);
      emit(state.copyWith(
          authStatusCheck: AuthStatusCheck.error, error: e.message));
    } catch (e, stack) {
      LogService.sendLog(TypeLog.errorInitApp,stack);
      emit(state.copyWith(
          authStatusCheck: AuthStatusCheck.error,
