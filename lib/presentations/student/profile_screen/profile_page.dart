...
class _UserProfilePageState extends State<UserProfilePage> {
  ...
  Future<void> _pickImage(bool galery) async {
    final pickedFile = await ImagePicker()
        .pickImage(source: galery ? ImageSource.gallery : ImageSource.camera);
    if (pickedFile != null) {
      _imageFile = File(pickedFile.path);
      final fileCompressed = await compressImage(imageFile: _imageFile!);
      profileEntity =
          profileEntity.copyWith(fileImageUrl: File(fileCompressed.path));
      setState(() {
        _edit = true;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: RefreshIndicator(
        ...
        child: BlocConsumer<ProfileBloc, ProfileState>(listener: (c, s) {
          ...
          if (s.profileStatus == ProfileStatus.saved) {
            ...
            Dialoger.showActionMaterialSnackBar(
                context: context, title: 'Изменения сохранены'.tr());
          }
              ...
              child: SingleChildScrollView(
                ...
                child: Stack(
                  ...
                  children: [
                        ...
                        child: Stack(
                          ...
                          children: [
                            ...
                            Container(
                              ...
                              child: _getAvatar(
                                  _imageFile, state.userEntity.avaUrl),
                            ),
                            Positioned(
                              bottom: 0,
                              right: 65,
                              child: Container(
                                ...
                                decoration: BoxDecoration(
                                    ...
                                    color: colorOrange,
                                    border: Border.all(
                                        color: colorWhite, width: 1.5)),
                                child: Icon(
                                    _editPhotoMode ? Icons.close : Icons.edit,
                                    color: colorWhite,
                                    size: 20),
                              ),
                            ),
                    ...
                    if (state.profileStatus == ProfileStatus.saving) ...{
                       Positioned(
                         top: -5,
                         right: 0,
                         child:  Center(
                        ...
                        child: SizedBox(
                          ...
                          child: Opacity(
                            ...
                            child: TextButton(
                                ...
                                onPressed: () {
                                  if (_edit) {
                                     context.read<ProfileBloc>().add(SaveNewDataUserEvent(editProfileEntity: profileEntity));
                                  }
                                }),
                          ),
                        ),
    ...
    if (imageFile == null) {
      if (urlAva.isEmpty) {
            ...
            child: CachedNetworkImage(
              ...
              progressIndicatorBuilder: (context, url, downloadProgress) =>
                  CircularProgressIndicator(
                      value: downloadProgress.progress, color: colorWhite),
        ...
        return CircleAvatar(
          ...
          child:
              Icon(Icons.hourglass_empty, color: colorWhite.withOpacity(0.5)),
        );
      }
    } else {
...
class _BodyInfoUserState extends State<BodyInfoUser> {
  ...
  void initState() {
    ...
    _aboutMiController = TextEditingController(
        text:
            widget.user.about_me.isEmpty ? 'Нет данных' : widget.user.about_me);
    _whoFindController = TextEditingController(
        text:
            widget.user.who_find.isEmpty ? 'Нет данных' : widget.user.who_find);
  }
  ...
  bool _fullDataProfile(EditProfileEntity profileEntity) {
    bool fullData = false;
    if (profileEntity.sex.isNotEmpty &&
        profileEntity.dateBirth.isNotEmpty &&
        profileEntity.urlAva.isNotEmpty) {
      fullData = true;
    }
    return fullData;
  }
  ...
  @override
  Widget build(BuildContext context) {
    return BlocConsumer<ProfileBloc, ProfileState>(listener: (c, s) {
      if (s.findSubwaysStatus == FindSubwaysStatus.added) {
        if (!_subways.contains(s.addedSubway)) {
          ...
          widget.edit.call(_profileEdited);
        }
      }
    }, builder: (context, state) {
            ...
            child: Column(
              ...
              children: [
                ...
                Row(
                  ...
                  children: [
                    Icon(Icons.phone_enabled_rounded,
                        color: textColorBlack(context), size: 12.0),
                    const Gap(5.0),
                ...
                Column(
                  ...
                  children: [
                    Text(
                      'Часто посещаемые станции метро:'.tr(),
                      style:
                          TStyle.textStyleVelaSansMedium(colorGrey, size: 16),
                    ),
                    const Gap(8.0),
                    Column(
                                ...
                                Container(
                                  decoration: BoxDecoration(
                                      ...
                                      color: theme == ThemeStatus.dark
                                          ...
                                          : colorWhite,
                                      border: Border.all(
                                          color: _getColorSubway(
                                              _subways[0].color),
                                          //todo list only with one item
                                          width: 1)),
                                  padding: const EdgeInsets.all(2),
                                  margin: const EdgeInsets.only(top: 3),
                                  child: Icon(Icons.directions_subway,
                                      color: _getColorSubway(_subways[0].color),
                                      //todo list only with one item
                                      size: 14),
                                ),
                                const Gap(10),
                                  ...
                                  child: InkWell(
                                      onTap: () {
                                        ...
                                        _profileEdited = _profileEdited
                                            .copyWith(subways: _subways);
                                        widget.edit.call(_profileEdited);
                                      },
                                      child: Icon(Icons.remove_circle_outline,
                                          size: 16, color: colorRed)),
                                )
                              ],
                            ),
                          );
                ...
                Column(
                  ...
                  children: [
                    Padding(
                      ...
                      child: Text(
                        'Кого ищу себе в группу (в напарники, в бенд)?',
                        style:
                            TStyle.textStyleVelaSansMedium(colorGrey, size: 16),
                      ),
                    ),
                    ...
                    Row(
                      ...
                      children: [
                        TextEdit(
                          controller: _whoFindController,
                          onChanged: (text) {
                            _profileEdited =
                                _profileEdited.copyWith(whoFindTeem: text);
                            widget.edit.call(_profileEdited);
                          },
                        )
                ...
                Column(
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        TextEdit(
                          controller: _aboutMiController,
                          onChanged: (text) {
                            _profileEdited =
                                _profileEdited.copyWith(aboutMe: text);
                            widget.edit.call(_profileEdited);
                          },
                        ),
                      ],
                ...
                GestureDetector(
                  onTap: () async {
                    ...
                    _dateBirth =
                        DateTimeParser.getDateToApi(dateNow: pickedDate);
                    _profileEdited =
                        _profileEdited.copyWith(dateBirth: _dateBirth);
                    widget.edit.call(_profileEdited);
                  },
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                  ...
                  children: [
                    ...
                    Text(
                      DateTimeParser.getDateFromApi(
                          date: widget.user.registration_date),
                      style: TStyle.textStyleVelaSansBold(
                          Theme.of(context).textTheme.displayMedium!.color!,
                          size: 18),
                    ),
                ...
                Row(
                  ...
                  children: [
                    ...
                    SelectKidsMenu(
                      ...
                      onKind: (hasKind) {
                        _profileEdited =
                            _profileEdited.copyWith(hasKind: hasKind as bool);
                        widget.edit.call(_profileEdited);
                      },
                    )
                  ],
                    ...
                    child: Row(
                      ...
                      children: [
                        Text('Удалить аккаунт'.tr(),
                            style: TStyle.textStyleVelaSansRegular(colorRed,
                                size: 16.0)),
                      ],
                    ),
                  ),
                ...
                children: [
                  ...
                  Row(
                    children: [
                      ...
                      Expanded(
                          child: Text(
                              'Заполни карточку целиком - получи БОНУСНЫЙ УРОК!',
...
class SelectKidsMenu extends StatefulWidget {
  const SelectKidsMenu(
      {super.key, required this.hasKind, required this.onKind});
    ...
    return DropdownButtonHideUnderline(
      child: DropdownButton2<String>(
        ...
        iconStyleData: IconStyleData(
          ...
          iconDisabledColor: Theme.of(context).textTheme.displayMedium!.color!,
        ),
